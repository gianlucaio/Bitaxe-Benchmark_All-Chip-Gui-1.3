...
### 🔍 New Features
- **Network Scanner**: Automatically discover all Bitaxe devices on your local network (10.x.x.x range). Click the 🔍 button next to the IP field to scan, then click any discovered device to auto-fill its IP. Shows response time for each device.
- **Live Monitor & Control**: 🆕 Real-time monitoring window without AXE OS! Click the 📊 Monitor button to open a dedicated window showing:
  - Live hashrate, power, efficiency
  - Chip & VR temperatures
  - Voltage, frequency, WiFi signal
  - Error rate tracking
  - Direct controls: adjust voltage/frequency, apply settings, restart device
  - Auto-refresh every second for true real-time monitoring
...
```bash
python3 BitaxeBenchGui_1_8.py
```

1. **Find your Bitaxe**: Click 🔍 **Scan Network** to discover devices, or enter IP manually
...
| 🔍 Scan Network | 🆕 **NEW!** Scan local network (10.x.x.x) for Bitaxe devices |
...
Click the **🔍 Scan Network** button to discover Bitaxe devices on your local network:

- Scans the 10.0.0.0/24 range (common Bitaxe subnet)
- Shows all responsive devices with their IP, hostname, and response time
- Verifies each device is actually a Bitaxe by checking the API
- Click any row to auto-fill the IP address field
- Perfect when your Bitaxe IP has changed via DHCP

### Live Monitor & Control 🆕 NEW!

Click the **📊 Monitor** button to open a real-time monitoring window:

**Live Metrics Displayed**:
- Hashrate (GH/s) with live chart
- Power consumption (W)
- Efficiency (J/TH)
- Chip temperature (°C)
- VR temperature (°C)
...
**Direct Controls**:
- Adjust voltage slider and apply immediately
- Adjust frequency slider and apply immediately
- Restart device button
- Auto-refresh every 1 second for true real-time monitoring
- No AXE OS web interface needed!

**Features**:
- Dark Bitcoin-themed UI matching the main app
- Live updating chart of hashrate over time
- Status indicators for connection health
- Close when done — doesn't affect ongoing benchmarks
...
| ⚖ Compare | Load two JSONs side-by-side to compare benchmarks |
| 📊 Monitor | 🆕 **NEW!** Open live monitoring window without AXE OS |
...
Every N steps (default 10), a partial JSON is saved as `bitaxe_benchmark_<ip>_<datetime>_PARTIAL.json`. If the benchmark is interrupted, you can load this file via ⏭ Resume and continue where you left off. Set interval to 0 to disable.
...
### Markdown (v1.7) — `bitaxe_benchmark_<ip>_<datetime>.md`
...
Open via ⚖ Compare. Load two benchmark JSONs (e.g., before/after a hardware mod) and view them side-by-side:
...
**✨ New Features**:
- **Network Scanner**: 🔍 Discover Bitaxe devices on local network (10.x.x.x), auto-fill IP with one click
- **Live Monitor & Control**: 📊 Real-time monitoring window without AXE OS — view hashrate, temps, power, error rate; adjust voltage/frequency directly; auto-refresh every second
...
If auto-save is enabled (default), a `_PARTIAL.json` file is saved every 10 steps. Press ⏭ Resume, select that file, and the benchmark continues from the last checkpoint.
...
### How do I find my Bitaxe if the IP changed?

Use the new **🔍 Network Scanner**! Click the scan button next to the IP field, and it will discover all Bitaxe devices on your 10.x.x.x network. Click any device to auto-fill its IP. No more router login needed!

### Can I monitor my Bitaxe without opening AXE OS?

Yes! Click the **📊 Monitor** button to open a dedicated real-time monitoring window. It shows hashrate, power, temps, voltage, frequency, error rate, and more — with auto-refresh every second. You can even adjust voltage/frequency and restart the device directly from the monitor window!
