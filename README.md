...
Enter the Bitaxe IP address (e.g. 192.168.1.100)
Set your starting voltage, frequency, steps and ceilings
(Optional) Load a saved preset or configure auto-save interval
Press ▶ Start Benchmark
When finished, the best configuration is applied automatically and results are saved as JSON + CSV + Markdown
GUI Overview
Configuration Panel
Field	Description
...
Reset frequency to start_f
Apply voltage / frequency to the device via PATCH /api/system
Restart the device
Wait for warm-up (adaptive or fixed)
Sample the device every 15 s for ~500 s total (~33 samples)
Record hashrate, temperature, VR temp, power, error rate
🆕 Thermal safety check: If temp rises >3°C in 30s, pause 2 min & retry
🆕 Auto-save: If step count is multiple of auto-save interval, save partial JSON
Advance frequency by f_step and repeat from step 2
After finishing all frequencies (or early-stop), advance voltage and repeat
...
errorPercentage — direct AxeOS field, matches the dashboard UI exactly (AxeOS 2.12+)
asicErrorRate — older field name used in some firmware forks
Delta of hashrateMonitor.asics[n].errorCount between consecutive samples — per-chip increment, used as fallback when neither field is available
