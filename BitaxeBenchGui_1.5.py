"""
Bitaxe All Model Hashrate Benchmark — GUI Edition  v1.5
...
class BitaxeBenchmark:
    ...
    def run(self):
        """
        2-D sweep: for every voltage level, test ALL frequencies from the
        starting value up to max_frequency (stepping by f_step).  Then move
        to the next voltage level and repeat from the starting frequency.

        This means every voltage × frequency combination is benchmarked, so
        the JSON contains the full picture — not just the diagonal that the
        old 1-D loop produced.

        A voltage level is skipped early only if a hard safety limit is hit
        (temperature, power, input voltage).  An unstable hashrate (ok=False)
        is recorded but does NOT stop the frequency sweep for that voltage —
        it is simply logged and the next frequency is tested.
        """
        try:
            ...
            while cur_v <= max_v:
                ...
                cur_f          = start_f
                safety_hit     = False   # hard limit → skip rest of this voltage
                ...
                while cur_f <= max_f:
                    ...
                    if avg_hr is not None:
                        # --- record result regardless of ok/not-ok ---
                        result = {
                            "coreVoltage":        cur_v,
                            "frequency":          cur_f,
                        ...
                        # hard safety limit hit — abandon this voltage level
                        self._log(
                ...
                # advance voltage for next outer iteration
                cur_v += v_step
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Label(
            banner, text="⛏  BITAXE ALL CHIP BENCHMARK v1.5",
            bg=C["accent"], fg="#111827",
